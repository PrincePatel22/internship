...
export default function Home() {
  ...
  const result = users && users.length;
  ...
  const handleDelete = async (id) => {
    ...
    if (choice === true) {
      try {
        let res = await axios.post("/api/deleteuser", {
          id: id,
        });
      } catch (error) {
        console.log(error);
  ...
  const handleExport = async () => {
    try {
      let res = await axios.post("/api/exportusers", {
        users: users,
      });
    ...
    if (filterUser) {
      ...
      for (let i = 0; i < filterUser.length; i++) {
        if (filterUser[i].status == "Y" && status == "Active") {
          ...
          (filterUser[i].status == "N" || filterUser[i].status == "Y") &&
      ...
      <main>
        ...
        >
          ...
          <div id="list1" className="dropdown-check-list">
            <span className="anchor">Select hobbies</span>
            <ul className="items">
              <li>
                <input
                  type="checkbox"
                    ...
                    <td>
                      <div>
                        ...
                        <button
                          ...
                          onClick={() =>
                            router.push("/EditUser/" + items.recid)
                          }
                        >
                          <FaUserEdit />
                        </button>
