...
app.post("/login", (req, res) => {
  ...
  const validation = `SELECT email FROM registration_prince WHERE email = "${email}" AND password = "${password}"`;

  dbConn.query(validation, (err, result) => {
    if (result.length > 0 && result[0].email === email) {
      const token = jwt.sign({ email: email }, secretKey);
      const sqlquery = `UPDATE registration_prince SET accesstoken="${token}" WHERE password = "${password}"`;
